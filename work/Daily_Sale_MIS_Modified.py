...
for whole_rows in ws.iter_rows(min_row=3):
...
df_temp = pd.read_excel(first_file, sheet_name='summary')

temp_i = 1
temp = list()
for col in ws.iter_cols(min_row=11, max_row=11, min_col=2, max_col=9):
    total = int((df_temp.iloc[2:, [temp_i]].sum())/1000)
    temp.append(total)
    temp_i += 1
print(temp)
...
for index, val in enumerate(temp):
    string_val = str[index] + '10'
    ws[string_val] = val
    print(f'{string_val} --> {val}')
...
def create_html(ws):
    ...
    for whole_row in ws.iter_rows(min_row=3):
        ...
        for row in whole_row:
            if isinstance(row.value, float):
                html += f'<td>{round(row.value, 0)}</td>'
                print(int(round(row.value, 0)))
            else:
